...
//typedef void(*)(void *a, void *b)   Swap;

typedef int *  p_int;
//函数指针，以后Swap类型的变量可以指向型如void func(void *a, void *b)的函数
...
{
    volatile int i = 10;  
    //忽略编译器的优化，每次对变量的访问都从变量的地址读取该值
    //一般在多线程程序中使用，针对可能被多个线程操作的资源
    //
    //
    // i5  双核4线程  
    // i7  四核8线程
...
int main(int argc, char **argv)
{
    //void a;   //编译器无法获知分配给a多大内存，wrong
    void *p;    //指针在32位下4字节，64位下8字节
...
int main(int argc, char **argv)
{
    ...
    for(i = 0; i < 100; ++i){
        for(j = 0; j < 100; ++j){
            if(j > 50){
                continue ;   //continue是跳出本次循环，进入到下一次循环
            }
