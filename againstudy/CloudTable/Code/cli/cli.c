...
char* get_table_name(char *string)
{
...
TableInfo* get_table_info_from_master(char* master_conn, char* table_name)
{
...
void update_table_info_list(CliCache* cliCache)
{
    ...
    for(i=0; i<size; i++)
    {
        ...
        TableInfo* newTableInfo = get_table_info_from_master(cliCache->master_conn,
                                                             oldTableInfo->table_name);
...
TableInfo* search_and_update_table_info(CliCache* cliCache, char* table_name)
{
...
boolean create_new_table(char* master_conn, char* table_name)
{
...
boolean add_new_region(char* master_conn, char* region_conn)
{
    boolean result = false;
...
boolean check_problem_region(char* problem_region_conn)
{
...
char* batch_put_data_to_table(CliCache* cliCache, TableInfo* tableInfo, ResultSet* resultSet)
{
...
ResultSet* query_table(CliCache *cliCache, TableInfo* tableInfo, char *row_key)
{
...
void show_table_metadata(TableInfo* tableInfo)
{
...
void show_master_metadata(CliCache* cliCache)
{
    if(cliCache->master_conn == NULL)
    {
        printf("%s.\n", ERR_MSG_NO_MASTER);
...
void load_cli_cache(char* conf_path)
{
...
char *cli_normalize_cmd(char *cmd)
{
...
void silent_mode(int argc, char** argv)
{
    if(argc == 2)
    {
