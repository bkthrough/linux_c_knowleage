...
char* m_get_value_by_key(char *file_path, char* target_key)
{
    char* value = NULL;
...
int get_int_value_by_key(char *file_path, char* target_key)
{
...
void flush_key_value(char* file_path, char* key, char* value)
{
    Buf* buf = init_buf();
    List* lines = get_lines_from_text_file(file_path);
    char* line = NULL;
...
char* key_and_value_list_to_line(char *key, List* valueList)
{
    char* value = list_to_string_by_token(valueList, STRING_SEPARATOR);
    return key_and_value_to_line(key, value);
}

SyncJob* create_sync_job(char* target_conn, char* table_name, 
...
boolean match_region(void *destRegionInfo, void *srcRegionConn)
{
...
RegionInfo* get_region_info(List* regionInfoList, char* region_conn)
{
    return list_find(regionInfoList, region_conn, match_region);
}

RegionInfo* create_region_info(char* region_conn)
{
...
char* m_region_info_to_string(RegionInfo *regionInfo)
{
    Buf* buf = init_buf();
...
char* m_region_info_list_to_string(List* regionInfoList)
{
    Buf* buf = init_buf();
...
List* load_region_info_list(char* file_path)
{
    List* regionInfoList = NULL;
    char* region_info_list_string = m_get_value_by_key(file_path, CONF_REGION_LIST_KEY);
    if(region_info_list_string != NULL)
        regionInfoList = string_to_region_info_list(region_info_list_string);
    else 
        regionInfoList = list_create();
    free2(region_info_list_string);
    return regionInfoList;
}

void flush_region_info_list(char* file_path, List* regionInfoList)
{
...
TabletInfo* create_tablet_info(RegionInfo *regionInfo, long long begin_timestamp, long long end_timestamp)
{
...
boolean match_tablet_info(void *tabletInfo_void, void *regionInfo)
{
    TabletInfo* tabletInfo = (TabletInfo*)tabletInfo_void;
    if(tabletInfo->regionInfo == regionInfo) 
        return true;
    else
        return false;
}

TabletInfo* get_tablet_info(List* tabletInfoList, RegionInfo* regionInfo)
{
    return list_find(tabletInfoList, regionInfo, match_tablet_info);
}

/* sample string: 127.0.0.1:8302,120000,12000 */
char* m_tablet_info_to_string(TabletInfo* tabletInfo)
{
    Buf* buf = init_buf();
...
/* sample string: 127.0.0.1:8302,120000,12000 */
TabletInfo* string_to_tablet_info(char* string)
{
    Tokens* tokens = init_tokens(string, STRING_SEPARATOR);
...
ReplicaQueue* create_replica_queue(int id)
{
...
void free_replica_queue(void* replicaQueue_void)
{
...
boolean match_replica_queue(void *replicaQueue_void, void *id)
{
    ReplicaQueue* replicaQueue = (ReplicaQueue*)replicaQueue_void;
    if(replicaQueue->id == (int)id) 
        return true;
    else
        return false;
}

ReplicaQueue* get_replica_queue(List* replicaQueueList, int id)
{
...
char* m_replica_queue_to_string(ReplicaQueue* replicaQueue)
{
    Buf* buf = init_buf();
...
/* sample string: 127.0.0.1:8302,120000,12000;127.0.0.1:8303,120000,12000 */
ReplicaQueue* string_to_replica_queue(int id, char* string)
{
...
TableInfo* create_table_info(char *table_name)
{
...
boolean match_table_info(void *tableInfo_void, void *table_name)
{
    TableInfo* table_info = (TableInfo*)tableInfo_void;
    if(match(table_info->table_name, (char*)table_name))
        return true;
    else 
        return false;
}

TableInfo* get_table_info(List* tableInfoList, char* table_name)
{
    return list_find(tableInfoList, table_name, match_table_info);
}

List* load_table_info_list(char *file_path)
{
...
void destory_table_info(void* tableInfo_void)
{
...
TableInfo* string_to_table_info(char* table_name, List* lines)
{
...
char* table_info_to_string(TableInfo *tableInfo)
{
    Buf* buf = init_buf();
...
char* table_info_list_to_string(List* tableInfoList)
{
    Buf* buf = init_buf();
...
void flush_table_info_list(char* file_path, List* tableInfoList)
{
    Buf* buf = init_buf();
    List* lines = get_lines_from_text_file(file_path);
    char* line = NULL;
    while((line = list_next(lines)) != NULL)
    {
        ...
        char* key = m_get_key(line);
        if(match(key, CONF_TABLE_INFO_LIST_KEY)) 
            continue;
...
char* get_conf_path_from_argv(int argc, char *argv[], char* default_conf_path)
{
    char *conf_path = NULL;
