...
void update_region_info(RegionInfo* regionInfo)
{
...
int cmp_region_info_void(void const* regionInfo1_void, void const* regionInfo2_void)
{
...
int get_tablet_used_size(RegionInfo* regionInfo, char* table_name)
{
    ...
    {
        /*ODO need to use the async mode*/
...
List* sort_region_info_list(List* regionInfoList)
{
...
boolean add_new_region_master(char* region_conn)
{
...
TabletInfo* create_new_tablet(RegionInfo *regionInfo, char* table_name)
{
...
boolean region_has_problem(RegionInfo* regionInfo)
{
...
boolean create_new_table_master(char* table_name)
{
...
boolean is_good_replica_queue(ReplicaQueue* replicaQueue, List* problemRegions)
{
...
List* find_src_tablets(TableInfo* tableInfo, TabletInfo* infectedTabletInfo, List* problemRegions)
{
    ...
    for(i=0; i<replica_queue_size; i++)
    {
        ...
        if(!is_good_replica_queue(replicaQueue, problemRegions)) 
            continue;
...
boolean start_sync_job(char* src_conn, char* target_conn, char* table_name,
...
boolean handle_infected_tablet(TabletInfo* infectedTabletInfo, ReplicaQueue* replicaQueue,
        TableInfo* tableInfo, List* problemRegions, List* regionInfoList)
{
    ...
    if(region_has_problem(firstRegionInfo)) 
        return false;
    ...
    else
    {
        ...
        if(infectedTabletInfo->end_timestamp == 0)
        {
            ...
            if(region_has_problem(secondRegionInfo))
                return false;
...
void handle_problem_regions(List* problemRegions, Master* master)
{
 ...
 *
 *    *  */
boolean check_problem_region_master(char* problem_region_conn)
{
    boolean result = false;
...
void update_master_info(Master* master)
{
    List* problem_regions = list_create();
    /*Update Region Info*/
...
void load_master(char *conf_path)
{
 ...
 *
 *   **/
RPCResponse* handler_master_request(char *cmd, List* params)
{
...
int main(int argc, char *argv[])
{
    ...
    logg(INFO,"The CloudTable Version is %s.",VERSION);
    /*0.9 master is only allow one region node*/
    logg(INFO,"The 0.1 Version only allows adding one region node.");
