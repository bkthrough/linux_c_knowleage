...
Tablet* get_tablet(List* tabletList, char *table_name)
{
...
boolean tablet_exist(List* tabletList, char *table_name)
{
...
Tablet* get_tablet_by_id(List* tabletList, int tablet_id)
{
...
long long get_last_flushed_id_from_conf(char* conf_path, char* tablet_folder)
{
...
void flush_tablets_info(char* file_path, List* tabletList)
{
...
Region* init_region_struct(char *conf_path)
{
...
int reload_wal_to_tablet(Wal* wal, List* tabletList)
{
...
void load_local_region(char *conf_path)
{
...
void check_and_flush_region(void)
{
    int used_size = 0;
...
RPCResponse* handler_region_request(char *cmd, List* params)
{
