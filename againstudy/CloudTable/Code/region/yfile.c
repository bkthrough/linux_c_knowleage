...
Trailer* create_trailer(int index_block_offset, char* table_name, size_t total_item_num, Key* firstKey,
...
void flush_trailer(Trailer *trailer, FILE *fp)
{
...
IndexBlock* resize_indexs(IndexBlock *indexBlock, int target_size)
{
...
IndexBlock* create_index_block(void)
{
...
void append_index(IndexBlock *indexBlock, size_t offset, size_t item_size, Key* lastKey,
...
void flush_index(Index *index, FILE *fp)
{
...
IndexBlock* load_index_block(int index_block_offset, FILE *fp)
{
...
void flush_index_block(IndexBlock *indexBlock, int begin_offset, FILE* fp)
{
...
void flush_item_list(IndexBlock *indexBlock, size_t begin_offset, ResultSet* resultSet, FILE* fp)
{
    int i=0, size = 0;
...
long long search_timestamp(IndexBlock* indexBlock, boolean begin)
{
    int i=0;
...
YFile* create_new_yfile(char* file_path, ResultSet* resultSet, char* table_name)
{
    ...
    Trailer *trailer = create_trailer(begin_index_block_offset, table_name, 
                                      total_item_num, firstKey, lastKey,
...
YFile* loading_yfile(char* file_path)
{
    ...
    if(cmp(yfile->trailer->magic, TRAILER_MAGIC, strlen(TRAILER_MAGIC))==false)
        return NULL;
    /*load the index*/
...
void free_data_block(DataBlock* dataBlock)
{
    if(dataBlock != NULL)
    {
...
DataBlock* load_data_block(YFile *yfile, int index_pos, FILE *fp)
{
...
int bsearch_indexs(int l, int r, Index** indexs, char* row_key, Key* firstKey)
{
    ...
    if(m-1>=0)
        between_result = between_keys(indexs[m-1]->lastKey, indexs[m]->lastKey, row_key);
    else 
        between_result = between_keys(firstKey, indexs[m]->lastKey, row_key);
    
    int cmp_result = cmp_key_with_row_key(indexs[m]->lastKey, row_key);
    if( cmp_result == 0 || between_result == true) 
        return m;
    else if(l == r ) 
        return -1;
    else if(cmp_result > 0) 
        return bsearch_indexs(l, m-1, indexs, row_key, firstKey);
    else
        return bsearch_indexs(m+1, r, indexs, row_key, firstKey);
}

boolean bloom_filter_key(YFile* yfile, char* row_key)
{
    return between_keys(yfile->trailer->firstKey, yfile->trailer->lastKey, row_key);
}

boolean bloom_filter_timestamp(YFile* yfile, long long begin_timestamp, long long end_timestamp)
{
    return match_by_timestamps(begin_timestamp, end_timestamp, yfile->trailer->begin_timestamp,
...
ResultSet* query_yfile_by_timestamp(YFile* yfile, long long begin_timestamp, long long end_timestamp)
{
    ...
    if(bloom_filter_timestamp(yfile, begin_timestamp, end_timestamp))
    {
        ...
        for(i=0; i<yfile->indexBlock->index_count;i++)
        {
            ...
            if(match_by_timestamps(index->begin_timestamp, index->end_timestamp,
