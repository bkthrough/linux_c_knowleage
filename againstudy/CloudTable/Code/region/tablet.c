...
int extract_tablet_id(char *table_folder)
{
...
Tablet* init_tablet_struct(int tablet_id, char *tablet_folder)
{
...
ResultSet* query_yfiles_by_row_key(List* yfileList, char *row_key)
{
...
ResultSet* query_yfiles_by_timestamp(List* yfileList, long long begin_timestamp, long long end_timestamp)
{
    ResultSet* resultSet = m_create_result_set(0, NULL);
    int i=0, size=list_size(yfileList);
    for(i=0; i<size; i++)
...
char* get_next_yfile_path(Tablet *tablet)
{
...
void refresh_tablet(Tablet *tablet, int hotnessValue)
{
    ...
    if(memstore_full(tablet->memstore))
    {
        ...
        pthread_mutex_lock(&tablet->flushing_mutex);
        /** Because sometime memstore data structure will be changed during the process, 
         * so use the flushed size as the defined mark,if the machine powers off during reset,
         * may have chance of losing data, TODO fix it in the later release **/
...
int get_disk_usage(Tablet *tablet)
{
    int disk_usage = 1;
