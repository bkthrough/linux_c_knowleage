...
/* 程序执行流程为基于请求链的传递，具体流程为通过解析函数，解析本次请求链；
  分析本次请求链，调用相关模块执行，生成响应头信息，以及定位文件信息
  发送响应头信息以及文件，更新日志记录
*/
typedef struct HttpRequest
{
	char method[20]; // 请求方法
	char *path; // 请求路径及资源名，若为CGI则为请求的可执行程序路径
	char *content; //　请求内容，面向POST和GET CGI请求
	int contentlength; //　请求内容的长度
	int			contenttype;	/* 回响的内容类型: */
	int			rangeflag;     /* 以下用于断点续传，分别表示断点起始点，结束点以及总长度*/
	long		rangestart;
	long		rangeend;
	long		rangetotal;	
	char	    prefix[20];
...
int getrequest(char *requestbuf,struct HttpRequest *prequest)
{
	...
	if(strcmp(prequest->method,"GET") == 0)
	{
		if(strchr(path, '?') == NULL)
		{
			// 先判断是否最后一个字符为/,如果为/则，挂加index.htm
